...
# ╔═╡ ca7f5646-08e1-11f1-9da1-c7654dd577e2
begin
	using Plots
	using PlutoUI
end

# ╔═╡ 5a2b749c-0324-4d36-acf8-f9791fb78b70
html"""
<h2 style="text-align:center; font-size: 2.2rem;">Modelling Spiral Arms Of Galaxies Using 2D Cellular Automata - Utility Notebook</h1>
"""

# ╔═╡ de1fd767-baaa-483a-a3a8-be64065fb3c8
html"""
<p style="margin-top:1rem;">This notebook contains the utility and helper functions required for the study of the spiral arm structure formation using a cellular automata model of galaxies. The code written in this notebook shall be used by the main notebook. This has been done for a cleaner presentation of the term paper</p>
"""

# ╔═╡ ddbf7fb4-ca74-4b59-863d-b26302774cce
html"""
<h2 style="text-align: center; font-size: 2rem;">The Grid</h2>
<p style="margin-top: 1.5em;">Since we have a galaxy, we shall be resorting to the use of a polar grid. The grid is essentially a two-dimensional disk that is divided into N rings. Each of these rings are further divided into cells of equal area.
</p>
"""

# ╔═╡ 9f75c4c1-c079-43c4-89c2-4deca68b7871
begin
...
# ╔═╡ c750de05-f59f-42d1-8d1f-c01b281a01df
...
# ╔═╡ 4a0aba5b-5da9-456e-b7a7-1d84cf032379
...
# ╔═╡ 5d6c17d4-ca7c-462d-8844-e3662cf763a0
html"""
<h2 style="text-align: center; font-size: 2rem;">Cell State Updation</h2>
<p style="margin-top: 1.2rem">The following 2 cells contains code to ease accessing and updating the values of the cells during the evolution of the galaxy</p>
"""

# ╔═╡ 48971e6e-d45a-47d1-8baa-b74d922dcf17
...
# ╔═╡ f437f750-fde5-4e2a-9a87-c60aa75298e4
function get_cell(grid::DiskGrid, ring::Int, cell_id::Int)
	return grid.cells[ring][cell_id]
end

# ╔═╡ 5f5c8e3b-5881-4520-a598-bcab82cc993d
html"""
<h2 style="text-align:center; font-size: 2rem;">Finding Neighbors</h2>
<p style="margin-top:1.2rem;">For this model, the neighbours for a particular cell are the cells that share a contiguous border.</p>
"""

# ╔═╡ ede4dbe1-039a-46a4-9ec5-f8d7e1a6cfc8
begin
# Strict overlap test (excludes touching at a point)
...
function find_neighbors(grid::DiskGrid, ring::Int, cell_idx::Int)
    ...
    if ring > 1
        for k in 1:cells_per_ring[ring-1]
            c = cells[ring-1][k]
            if intervals_overlap(θ1, θ2, c.θ_start, c.θ_end)
                push!(neighbors, (ring-1, k))
            end
        end
    end

	# Neighbours in outer ring
...
# ╔═╡ 0db0ab59-d2b5-4b81-8394-86710287f869
...
# ╔═╡ c8ae9514-30d0-4cb8-b18e-94b1453790ff
html"""
<h2 style="text-align:center;">Evolution Of The Model</h2>
"""

# ╔═╡ d9ea7c98-b8b9-4b11-b33e-c1e5b40db451
    ...
    for ring in 1:grid.num_rings
        for cell_id in 1:grid.cells_per_ring[ring]
            ...
            if current_state == Quiescent || 
               (current_state == Refractory && current_age < τᵣ)

                P = Pst
                if current_state == Refractory && current_age < τᵣ
                    P = Pst * (current_age / τᵣ)
                end
                ...
                if has_sf && rand() < P
                    ...
                    new_age = 0
                elseif new_state == Quiescent && rand() < Psp
                    new_state = StarForming
                    new_age = 0
                end
            end
...
# ╔═╡ 2b975df9-752a-4973-9ca3-105018370e6d
...
# ╔═╡ 65a02f96-8765-44a3-a74f-4b64a3ce3fb2
begin
	grid = create_disk_grid(49, 4.0, 18.0, 10)
	plot_disk(grid, "")
end

# ╔═╡ 7658fa4d-8144-486b-98d8-723ed8e141a8
plot_neighbourhood(grid, 2, 1)

# ╔═╡ 4dc22e44-f83d-45cf-8264-b51f6e2dbe36
function animate_evolution(grid, steps, Pst, Psp, τᵣ)
    ...
    gif(anim, "disk_evolution.gif", fps = 15)
end


# ╔═╡ f4c097b4-b17f-4238-8d8a-0938184407da
begin
# Solid body rotation (no shear)
rotation_curve_solid(r) = 0.5

# Flat galaxy-like rotation (v ≈ const → Ω ∝ 1/r)
rotation_curve_flat(r) = 1.0 / r

# Keplerian rotation (Ω ∝ r^(-3/2))
rotation_curve_kepler(r) = sqrt(1.0 / r^3)
end

# ╔═╡ 86cdfa41-2728-4b93-a2bd-3432e03da784
...
# ╔═╡ 8ffa84dc-e01f-4037-b7a8-d8f2bf666e04


# ╔═╡ 7e28250e-ffc7-4801-8b56-b20159485166


# ╔═╡ a8fd56ac-eb4b-4777-aded-a1a8f6a79b8b
function simulate_with_rotation(;
    num_rings::Int = 49, 
    r_inner::Float64 = 2.0,
    r_outer::Float64 = 20.0,
    Pst::Float64 = 0.28,
    Psp::Float64 = 0.0001,
    τr::Int = 11,
    rotation_curve::Function = rotation_curve_flat,
    dt::Float64 = 1.0,
    n_steps::Int = 400,
    fps::Int = 10
)
    ...
    # Create grid
    grid = create_disk_grid(num_rings, r_inner, r_outer, 25)

    # Seed 1% star-forming cells
    for r in 1:grid.num_rings
    ...
    # IMPORTANT: return animation instead of just saving
    gif(anim, fps=fps)
end


# ╔═╡ d9140320-45c3-4cca-8e64-07df26dfb089
simulate_with_rotation()

# ╔═╡ 0d114d29-eb0e-45e8-8518-a9dab4c2e624
